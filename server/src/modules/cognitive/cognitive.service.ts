...
export class CognitiveService {
  ...
  async updateDimension(userId: string, dto: UpdateDimensionDto) {
    ...
    // 更新维度
    const dimensionIndex = cognitiveMap.dimensions.findIndex(
      (d) => d.name === dto.dimension,
    );

    if (dimensionIndex >= 0) {
      // 更新现有维度（累加分数，最大100）
      const newScore = Math.min(
        cognitiveMap.dimensions[dimensionIndex].score + dto.score,
        100,
      );
    ...
    await this.cognitiveMapRepository.save(cognitiveMap);
    this.logger.log(
      `Updated dimension ${dto.dimension} for user ${userId} with score ${dto.score}`,
    );
  ...
  private analyzeCognitiveMaps(
    ...
    for (const dimName of dimensionNames) {
      ...
      // 共同优势: 平均分 >= 70
      if (avgScore >= 70) {
        commonStrengths.push(
          `${dimName}: 平均分 ${avgScore.toFixed(1)}`,
        );
      }
